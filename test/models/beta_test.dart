...
void main() {
  ...
  setUp(() => {
    betaModel = BetaModel(numberOfInterventions: 2, a: 1.0, b: 1.0, random: Random(0))
  });
  ...
  test('Equal outcomes should lead to around 50% chance', () {
    ...
    betaModel.sample();
    expect(listAlmostEquals(betaModel.maxProbabilities(), [0.5, 0.5], testEpsilon), true);
  });
  ...
  test('Numerical Example', () {
    ...
    betaModel.sample();
    expect(listAlmostEquals(betaModel.maxProbabilities(), [0.2, 0.8], testEpsilon), true);
  });

  test('Samples should be identical if using same random seed', () {
    var beta = BetaModel(numberOfInterventions: 2, a: 2.9, b: 3.7, random: Random(0));
    // Without history samples will be empty
    beta.history = createDataFrame({0: [0], 1: [1]});
    beta.sample();
