...
void main() {
  ...
  test('Numerical example', () {
    ...
    gaussianModel.history =
        createDataFrame({0: alternateOutcomes, 1: alternateOutcomes});
    gaussianModel.sample();
    expect(
        listAlmostEquals(
            gaussianModel.maxProbabilities(), [0.5, 0.5], testEpsilon),
    ...
    expect(
        listAlmostEquals(
            gaussianModel.maxProbabilities(), [0.3, 0.7], testEpsilon),
        true);
  });

  test('Samples should be identical if using same random seed', () {
    var gaussian = GaussianModel(
        numberOfInterventions: 2, mean: 2.9, l: 3.7, random: Random(0));
    // Without history samples will be empty
    gaussian.history = createDataFrame({
      0: [0],
      1: [1]
    });
    gaussian.sample();
  ...
  test('Samples should be different if using different random seed', () {
    ...
    expect(secondSample.isEmpty, false);
    for (int i = 0; i < 2; i++) {
      expect(listAlmostEquals(firstSample[i], secondSample[i], testEpsilon),
          false);
    }
  });
}
